...
const chatApi = {
  ...
  updateChat: (data) => {
    try {
      ...
      if (error.response.status === 400) {
        ...
        toast.error('Can not found admin!')
      }
    }
  },
  ...
  changeAdmin: async (data) => {
    try {
      await axiosClient.post(`${BASE_URL}/updateChangeAdmin`, data)
      toast.success('Change Admin successfully!!!!')
    } catch (error) {
      if (error.response.status === 400) {
        toast.error('User not found!')
      }else if (error.response.status === 409) {
  ...
  leaveGroup: async (data) => {
    try {
      await axiosClient.post(`${BASE_URL}/removeFromChat`, data)
      toast.success('Leave Group successfully!!!!')
    } catch (error) {
      if (error.response.status === 400) {
        toast.error('Can not leave group beacause you are admin!')
      }
    }
  },
  downloadFile: (data) => {
    try {
      const res = axiosClient.post(`${BASE_URL}/getFileChatDownload`, data)
      return res
    } catch (error) {
      if (error.response.status === 400) {
        toast.error('User not found!')
