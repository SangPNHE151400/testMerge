...
const attendanceApi = {
  ...
  exportAttendanceUserDetail: (user_id, date) => {
    try {
      const response = axiosClient.get(`${BASE_URL}/exportAttendanceUserDetail`, {
        params: {
          user_id: user_id,
          date: date
        }
  ...
  exportIndividualEvaluate: (userId, month, year) => {
    try {
      const response = axiosClient.post(`${BASE_URL}/exportIndividualEvaluate`, {
        params: {
          userId: userId,
          month: month,
          year: year
