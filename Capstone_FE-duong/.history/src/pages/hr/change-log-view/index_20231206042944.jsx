...
const ChangeLogView = () => {
  ...
  const commitChanges = async ({ added, deleted }) => {
      ...
      setHoliday((prevData) => {
        ...
        if(deleted !== undefined){
          newData = holiday.filter((holidays) => holidays.id !== deleted)
          console.log('asd');
        }
        return newData
      })
    ...
    //   setData((prevData) => {
    //     let newData = [...prevData]
    //     newData = data.filter((holiday) => holiday.id !== deleted)
    //     console.log(deleted);
    //     return newData
    //   })
    // }
  ...
  return (
    <Paper>
      <Scheduler data={holiday} height="auto">
        <ViewState />
