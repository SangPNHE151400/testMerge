...
const EvaluateReport = () => {
  ...
  const handleExportIndividualEvaluate = async () => {
    const res = await attendanceApi.exportIndividualEvaluate(currentUser?.accountId, format(month, 'MM'), format(month, 'yyyy'))
    console.log(res);
  ...
  return (
    <>
      <Paper style={{ padding: '20px' }}>
        ...
        <Grid item xs={6} width='48%' mt={2}>
        <Button onClick={handleExportIndividualEvaluate} variant="contained" sx={{bgcolor: 'yellow', color: '#000'}}>Export</Button>
        </Grid>
        {
          evaluate !== '' ? <>
          <TableContainer
