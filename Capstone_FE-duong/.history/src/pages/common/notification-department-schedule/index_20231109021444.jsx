...
const NotificationDepartMentScheduleList = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllNoti = async () => {
      const response = await axiosClient.get(`${BASE_URL}/getNotificationByDepartment`, {
        params: {
          userId: userId
        }
      })
        ...
        return (
          <Box
            ...
            <Box
              ...
              <Button
                variant="contained"
                onClick={() => handleHidden(params.row)}
                style={{ fontSize: '12px' }}>
                Hidden
              </Button>
            </Box>
  ...
  return (
    <>
      <Header title="DEPARTMENT SCHEDULED" />
      <DataTableDepartMent rows={allNoti} columns={columns} isLoading={isLoading} />
    </>
  )
}

export default NotificationDepartMentScheduleList
