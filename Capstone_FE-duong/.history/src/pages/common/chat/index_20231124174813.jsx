...
const Chat = () => {
  ...
  const imgurlAvatar = async () => {
    console.log('allChatList:', allChatList) // Log the entire allChatList object
    try {
      const downloadURLPromises = allChatList.map(async (item) => {
        if (item.avatar && Array.isArray(item.avatar)) {
          // If avatar is an array, map over its elements
          ...
          // Use Promise.all to wait for all avatar promises to resolve
          return Promise.all(avatarPromises)
        } else {
          // If avatar is not an array, or it's undefined/null, return null
      ...
      const result = allChatList.map((obj, index) => {
        return {
          ...obj,
          // If avatar is an array, assign the corresponding array of download URLs
              ...
              <>
                {item?.isGroupChat == 'false' ? (
                  ...
                  >
                    <Stack p={1} display="flex" direction="row" spacing={1} mb={1}>
                        {
                          item.avatar.length > 2 ? item.avatar.map((image) => (
                            <>
                            <AvatarGroup max={3}>
