...
const Chat = () => {
  ...
  useEffect(() => {
    const fetchAllUserSingleChat = async () => {
      const response = await chatApi.getUserSingleChat(currentUserId)
  ...
  const handleAutocompleteChange = (event, newValue) => {
    ...
    if (newValue) {
      ...
      if (!selectedUserSingleChat.some((user) => user.accountId === newValue.accountId)) {
        // If not, push it into the array
        setSelectedUserSingleChat((prevSelected) => [...prevSelected, newValue]);
      }
    }
  };

  console.log(allUserSingleChat);
