...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    setSelectedUserGroup(isActiveUser?.user ? isActiveUser.user.map((item) => item.accountId) : []);
  }, [isActiveUser])
  ...
  const handleUpdateChat = async () => {
    ...
    setAllChatList((prev) => {
      return prev.map((chat) =>
        chat.chatId === res.chatId ? res : chat
      );
    });
  ...
  const handleLeaveGroup = () => {
    ...
    setAllChatList((prev) => {
      return prev.filter((chat) =>
        chat.chatId !== isActiveUser?.chatId
      );
    });
    handleCloseChangeAdmin()
  }
        ...
        <Box sx={style}>
          ...
          <>
            ...
            <Autocomplete
              ...
              value={selectedUserGroup ? selectedUserGroup : []}
              getOptionLabel={(option) => option.username}
              onChange={(event, newValue) =>
          ...
          <>
            <FormControl sx={{ mt: 2, width: '100%' }}>
              ...
              <Select
                ...
                {
                  isActiveUser && isActiveUser?.user.map((item) => (
                    <MenuItem key={item.accountId} value={item.accountId}>{item.username}</MenuItem>
                  ))
                }
              </Select>
            </FormControl>
