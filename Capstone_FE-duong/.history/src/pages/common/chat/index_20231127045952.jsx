        ...
        <Box sx={style}>
          ...
          <>
            ...
            <Autocomplete
              ...
              id="tags-outlined"
              options={allUser}
              // defaultValue={userGroupUpdate.length > 0 && allUser.length > 0 && allUser.filter(user => {
              //   return userGroupUpdate.some(groupUser => groupUser.id === user.accountId);
              // })}
              value={selectedUser.accountId}
              getOptionLabel={(option) => option.username}
              onChange={(event, newValue) =>
          ...
          <Box display="flex" justifyContent="flex-end" gap="5px" mt={3}>
            <Button variant="contained" sx={{ bgcolor: 'green' }} onClick={handleCreateNewChat}>
              Update
            </Button>
            <Button variant="contained" sx={{ bgcolor: 'red' }} onClick={handleCreateNewChat}>
              Leave Group
            </Button>
          </Box>
        </Box>
