...
const BookRoom = () => {
  ...
  useEffect(() => {
    // Update the data structure to include the 'startDate' property
    const updatedData = department.map((item) => {
      return { text: item.departmentName, departmentId: parseInt(item.departmentId) }
    })

    // Set the updated data back into the state
    setModifiedDepartment(updatedData)
  }, [department])
